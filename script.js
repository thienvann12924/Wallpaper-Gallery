...
function setupCategoryNavigation() {
    ...
    dropdownItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            switch(currentCategory) {
                case 'anime':
                    titleText = 'Anime Collection';
                    iconClass = 'fas fa-dragon';
                    break;
                case 'girl':
                    titleText = 'Girls Collection';
                    iconClass = 'fas fa-heart';
                    break;
                case 'landscape':
                    titleText = 'Landscape Collection';
                    iconClass = 'fas fa-mountain';
                    break;
                case 'cyberpunk':
                    titleText = 'Cyberpunk Collection';
                    iconClass = 'fas fa-city';
                    break;
                case 'cute':
                    titleText = 'Cute Collection';
                    iconClass = 'fas fa-heart';
                    break;
                case 'pets':
                    titleText = 'Pets Collection';
                    iconClass = 'fas fa-paw';
                    break;
                case 'all':
                default:
                    titleText = 'All Collection';
        ...
        if (sampleApps.length === 0) {
            filesGrid.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; color: white; padding: 40px;">
                    ...
                    <p style="font-size: 1.2rem; opacity: 0.8;">The Wallpaper is not shared yet</p>
                    <p style="font-size: 1rem; opacity: 0.6; margin-top: 10px;">Be the first to share a wallpaper!</p>
                </div>
            `;
            updatePagination(0);
            return;
    ...
    modalContent.innerHTML = `
        ...
        <div class="image-modal-content">
            ...
            <div class="image-instruction">
                <p>Hold (in mobile) or right-click (in desktop) to save</p>
            </div>
        </div>
    `;
...
function downloadApp(app) {
    ...
    if (app.downloadLink) {
        ...
        alert('Link download is not set for this wallpaper.');
    }
}
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  audio.play().catch(e => {
    console.warn("Operator blocked autoplay:", e);
  });
});